...
Array.from(document.getElementsByClassName('playListPlay')).forEach((element) => {
    element.addEventListener('click', (e) => {
        ...
        download_music.href = `audio/${index}.mp3`;  // Set download link
        ...
        song_title.forEach((ele) => {
            ...
            download_music.setAttribute('download', songName); // Set download filename
        });
...
music.addEventListener('ended', () => {
    ...
    wave.classList.remove('active2');
    index++;
    if (index > songs.length) index = 1; // Loop back to first song

    music.src = `audio/${index}.mp3`;
    download_music.href = `audio/${index}.mp3`;  // Update download link
    poster_master_play.src = `img/${index}.jpg`;
    music.play();
    ...
    song_title.forEach((ele) => {
        let { songName } = ele;
        title.innerHTML = songName;
        download_music.setAttribute('download', `${songName}.mp3`); // Update filename for download
    });
